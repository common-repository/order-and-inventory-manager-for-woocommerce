 ...
 * DataLowStock class
 *
 * Handles products having low stock
 *
 * @since    1.0.0
 */
...
/**
 * DataLowStock Handles products having low stock
 */
class DataLowStock extends WP_List_Table
{
    public  $total_pages ;
    public  $currency ;
    ...
    public function table_data()
    {
        ...
        $post_in = get_post_meta( $_REQUEST['supplier_id'], 'oimwc_supplier_purchase_order_data', true );
        
        if ( $post_in ) {
            $purchase_data = implode( ',', array_keys( $post_in ) );
        } else {
            $purchase_data = '';
        }
        ...
        if ( isset( $_REQUEST['supplier_id'] ) && sanitize_text_field( $_REQUEST['supplier_id'] ) !== "all" ) {
            ...
            $temporary_product = $wpdb->get_results( 'SELECT * FROM ' . $wpdb->prefix . 'oimwc_temp_product WHERE supplier_id=' . $_REQUEST['supplier_id'] . ' AND order_id = 0 ' );
            $all_supplier = true;
        } else {
            $all_supplier = false;
            $temporary_product = $wpdb->get_results( 'SELECT * FROM ' . $wpdb->prefix . 'oimwc_temp_product  WHERE order_id = 0 ' );
        }
        ...
        if ( $_REQUEST['show_all_product'] == 1 ) {
            ...
            $query = 'SELECT A.post_id FROM ' . $wpdb->prefix . 'postmeta AS A 
                WHERE A.meta_key = "oimwc_supplier_id" AND A.meta_value = ' . $_REQUEST['supplier_id'];
            ...
            // Fetch products supplier wise.
            $query = 'SELECT A.post_id FROM ' . $wpdb->prefix . 'postmeta AS A 
                WHERE A.meta_key = "oimwc_supplier_id" AND A.meta_value = ' . $_REQUEST['supplier_id'] . ' AND A.post_id IN ( ' . implode( ',', $all_post_ids ) . ' )';
            $post_ids = $wpdb->get_col( $query );
            $post_ids = array_merge( $additional_supplier_ids, $variable_supplier_ids, $post_ids );
            // For check show in low stock or not.
            $query = 'SELECT A.post_id FROM ' . $wpdb->prefix . 'postmeta AS A 
        ...
        if ( $additional_product ) {
            $args['post__in'] = $additional_product;
        ...
        if ( $post_in ) {
            ...
            $args = '';
            oimwc_supplier_low_stock_count( 0, $request_supplier_id );
        ...
        if ( $_REQUEST['supplier_id'] == '' || $_REQUEST['supplier_id'] == 'all' ) {
            $data = array();
        } else {
        ...
        if ( count( $temporary_product ) > 0 && $paged == 1 ) {
        ...
        switch ( $column_name ) {
            case 'thumb':
                return $item[$column_name] . sprintf(
                    '<input type="hidden" name="purchase_order_data" value="%s" /><input type="hidden" class="productId" name="productId[]" value="%s" /><div class="mobile_prod_info"><div>%s</div><div class="%s">%s</div></div>',
                    $item['purchase_data'],
                    $item['id'],
            ...
            case 'amount':
                ...
                if ( !isset( $_REQUEST['supplier_id'] ) || isset( $_REQUEST['supplier_id'] ) && (sanitize_text_field( $_REQUEST['supplier_id'] ) == "all" || sanitize_text_field( $_REQUEST['supplier_id'] == 0 )) ) {
                    ...
                    if ( $item['temp_product'] == 1 ) {
                        $qty_column = '<div class="temp_product"><span>' . __( 'Qty', 'order-and-inventory-manager-for-woocommerce' ) . ': ' . $item['qty'] . '</span><input type="button" class="button btnTempRemoveProduct btn_' . $item['id'] . '" data-id="' . $item['id'] . '" value="' . __( 'Remove', 'order-and-inventory-manager-for-woocommerce' ) . '" /></div>';
                    } else {
                    ...
                    $qty_column = '<div class="' . $cls . '" data-tip="' . $msg . '"><input type="text" class="arrived_qty_handler" data-warning="' . $item['warning_level'] . '" data-stock="' . $item['product_stock'] . '" data-id="' . $item['id'] . '" name="product[' . $item['id'] . '][qty]" /></div>';
                    ...
                    $qty_column .= '<div class="product_calc"><span data-price="' . $item['non_format_purchase_price'] . '" class="amount">0</span> <span class="currency">' . $item['supplier_currency'] . '</span></div>';
            ...
            case 'product_info':
                ...
                return sprintf(
                    '<div class="' . $class . '"><div>%1$s</div>%2$s<div class="product_sku">%3$s: %4$s</div><div>%5$s: %6$s</div></div>',
                    $item['product_name'],
                    $variant,
